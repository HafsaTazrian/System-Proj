        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                if file_path.lower().endswith('.webm'):
                    temp_wav_path = os.path.join(app.config['UPLOAD_FOLDER'], f"{uuid.uuid4()}.wav")
                    if not convert_webm_to_wav(file_path, temp_wav_path):
                        os.remove(file_path)
                        return jsonify({'error': 'Failed to convert webm to wav'}), 500
                    features = feature_extraction(temp_wav_path)
                else:
